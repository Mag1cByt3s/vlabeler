...
nix develop
vlabeler-run                          # same wrapper as `nix run`
./gradlew packageDistributionForCurrentOS
```

### HiDPI scaling

The wrapper applies an explicit Compose density via `-Dvlabeler.uiScale=N`.
Default is `1.5`; override per launch:

```sh
VLABELER_UI_SCALE=2.0 nix run
VLABELER_UI_SCALE=1.0 nix run         # no extra scaling
```

To change the default, edit `defaultUiScale` near the top of `flake.nix`.

### What the flake does

The flake is a wrapper around `./gradlew`, not a full Nix package — Gradle still
fetches dependencies as usual. The wrapper handles a few NixOS quirks:

1. **GTK file picker would crash the app** (`GLib-GIO-ERROR: Settings schema
   'org.gtk.Settings.FileChooser' is not installed`). Fixed by exporting
